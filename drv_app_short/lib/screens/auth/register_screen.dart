...
class _RegisterScreenState extends State<RegisterScreen> {
    ...
    try {
      ...
      for (var entry in docs.entries) {
        if (entry.value != null) {
          ...
          final expiryDate = entry.key == 'dl_front' || entry.key == 'dl_back'
              ? _licenseExpiry
              : entry.key == 'insurance'
                  ? _insuranceExpiry
                  : null;
            ...
            child: ElevatedButton(
              onPressed: _loading ? null : () {
                ...
                if (_currentStep == 4) {
                  ...
                  if (_insuranceExpiry == null) { _showSnack('Select insurance expiry date', error: true); return; }
  ...
  Widget _buildStep5() {
    return _stepContainer('Vehicle Documents', 'Upload RC and Insurance', [
      ...
      const SizedBox(height: 12),
      _datePicker('Insurance Expiry Date', _insuranceExpiry, (d) => setState(() => _insuranceExpiry = d)),
      const SizedBox(height: 12),
